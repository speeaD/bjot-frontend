...
export default function CmsPage() {
  ...
  return <main className="min-h-screen bg-[#f5f7f6] px-4 pb-10 pt-5 sm:px-6 lg:px-8"><div className="mx-auto max-w-[1440px]">
    ...
    <section className="mb-5 grid grid-cols-1 gap-3 md:grid-cols-3"><Status icon={<FileText />} label="Landing data" value={loading ? "Loading..." : `${sections.length} sections`} detail="Read from the admin content API" tone="mint" /><Status icon={<Trophy />} label="Social proof" value={loading ? "—" : `${staff.length} staff · ${testimonials.length} stories`} detail="Loaded with CMS records" tone="orange" /><Status icon={<Megaphone />} label="Active banner" value={loading ? "—" : bannerLive ? "Visible to candidates" : "Banner paused"} detail={loading ? "Loading announcement" : bannerLive ? "Announcement is enabled" : "Announcement is hidden"} tone="green" /></section>
    {loading && <DashboardContentLoading label="Loading landing page content" />}
    <div hidden={loading}>
