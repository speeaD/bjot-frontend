...
import { Trophy, Medal, Award, Filter, Target, Loader2 } from "lucide-react";
...
// No Props — everything fetched client-side
export default function LeaderboardClient() {
  ...
  // ─── Leaderboard calculation (unchanged logic) ────────────────────────────
  const leaderboard = useMemo(() => {
    ...
    filteredSubmissions.forEach((submission) => {
      ...
      const email = submission.quizTakerId.name;
            ...
            <div className="bg-white rounded-lg shadow">
              ...
              <div className="p-6 border-b border-gray-200">
                ...
                <div className="flex gap-4 mb-4">
                  <button
                    onClick={() => { setViewMode("global"); setSelectedQuiz(""); }}
                    className={`px-6 py-2 rounded-lg font-medium transition-colors ${
                      viewMode === "global" ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                    }`}
                  >
                    Global Leaderboard
                  </button>
                  <button
                    ...
                    className={`px-6 py-2 rounded-lg font-medium transition-colors ${
                      viewMode === "per-quiz" ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                    }`}
                  >
                    Per Exam
                  </button>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors ${
                        selectedQuestionSet === null ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                      }`}
                    >
                      All Sets
                    </button>
                    {[1, 2, 3, 4].map((setNum) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors ${
                          selectedQuestionSet === setNum ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                        }`}
                      >
                        Set {setNum}
                      </button>
                    ...
                    {leaderboard[1] && (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="bg-white rounded-lg p-4 shadow-md text-center w-40">
                          ...
                          <div className="text-sm text-gray-600 mb-2 truncate">{leaderboard[1].email}</div>
                    ...
                    {leaderboard[0] && (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="bg-white rounded-lg p-4 shadow-lg text-center w-40">
                          ...
                          <div className="text-sm text-gray-600 mb-2 truncate">{leaderboard[0].email}</div>
                    ...
                    {leaderboard[2] && (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="bg-white rounded-lg p-4 shadow-md text-center w-40">
                          ...
                          <div className="text-sm text-gray-600 mb-2 truncate">{leaderboard[2].email}</div>
                          <div className="text-lg font-semibold text-blue-600">
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {leaderboard.map((entry) => (
                          <tr key={entry.accessCode} className="hover:bg-gray-50">
                            ...
                            {selectedQuestionSet === null ? (
                              <>
                                <td className="px-6 py-4">
                                  <span className={`px-3 py-1 rounded-full text-sm font-semibold ${getRankBadge(entry.rank)}`}>
                                    {entry.averageScore.toFixed(1)}%
                                  </span>
                                </td>
                                <td className="px-6 py-4 text-sm text-gray-900">{entry.totalQuizzes}</td>
                                <td className="px-6 py-4 text-sm font-semibold text-green-600">{entry.totalPoints}</td>
                              </>
                            ) : (
                              <>
                                <td className="px-6 py-4">
