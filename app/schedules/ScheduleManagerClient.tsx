...
const dayName = dayMap[day]; // ✓ No error
...
export default function ScheduleManagerClient({ 
  ...
  const [questionSets, setQuestionSets] = useState<any[]>([]); // You'll need to fetch this from your API
  ...
  // Set initial schedule for selected department
  useEffect(() => {
    console.log('All schedules:', allSchedules);
    const deptSchedule = allSchedules.find(s => s.department === selectedDepartment);
    setSchedule(deptSchedule || null);
  }, [selectedDepartment, allSchedules]);
  ...
  const loadSchedule = async () => {
    ...
    try {
      ...
      const newSchedule = response || null;
      setSchedule(newSchedule);
      
      // Update allSchedules array
      if (newSchedule) {
  ...
  const handleAddClass = () => {
    ...
    const updatedSchedule = schedule?.weeklySchedule || [];
  ...
  const [groupedSchedule, setGroupedSchedule] = useState<Record<DayOfWeek, ClassSession[]>>({
    0: [],
    1: [],
    2: [],
    3: [],
